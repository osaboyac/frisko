						...
						<div class="dataTable_wrapper">
							<table class="table table-striped table-bordered table-hover" id="dataTables-example">
								...
								<tr>
									...
									<td><?= h($tipo) ?></td>
									<td class="actions">
										<?= $this->Html->link(__(''),['action'=>'edit', $docseries->id],['class'=>'fa fa-edit btn btn-warning btn-xs']) ?>
										<?= $this->Form->postLink(__(''),['action' => 'delete', $docseries->id], ['confirm' => __('Está seguro de eliminar el registro # {0}?', $docseries->id),'class'=>'fa fa-times btn btn-danger btn-xs']) ?>
									</td>
								</tr>
								<?php endforeach; ?>
								</tbody>
