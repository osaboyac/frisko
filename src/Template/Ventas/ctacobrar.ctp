						...
						<div class="dataTable_wrapper">
							<table class="table table-striped table-bordered table-hover" id="dataTables-example">
								...
								<tr>
									...
									<td class="actions">
										<button type="button" class="btn btn-info btn-circle" onClick="AgregarFactura('<?= $venta->socio->id ?>','<?= $venta->socio->nombre ?>','<?= $venta->id ?>','<?= $venta->serie.'-'.$venta->numero ?>','<?= $venta->grantotal ?>','<?= $venta->fecha->format('Y-m-d') ?>');"><i class="fa fa-check"></i></button>
									</td>
								</tr>
								<?php endforeach; ?>
								</tbody>
...
<script>
	function AgregarFactura(socio_id, socio_nombre, venta_id, serie_numero, grantotal, fecha){
