				...
				<div class="dataTable_wrapper">
					<table class="table table-striped table-bordered table-hover" id="dataTables-example">
						...
						<tr>
							...
							<td class="actions">
								<?= $this->Html->link(__(''),['action'=>'printer', $ordenCompra->id],['class'=>'fa fa-print btn btn-default btn-xs','title'=>'Imprimir']) ?>
								<?= $this->Html->link(__(''),['action'=>'edit', $ordenCompra->id],['class'=>'fa fa-edit btn btn-warning btn-xs','title'=>'Editar']) ?>
								<?= $this->Form->postLink(__(''),['action' => 'delete', $ordenCompra->id], ['confirm' => __('Está seguro de eliminar el registro # {0}?', $ordenCompra->id),'class'=>'fa fa-times btn btn-danger btn-xs']) ?>
							</td>
						</tr>
						<?php endforeach; ?>
						</tbody>
