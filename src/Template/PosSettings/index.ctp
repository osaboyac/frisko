						...
						<div class="dataTable_wrapper">
							<table class="table table-striped table-bordered table-hover" id="dataTables-example">
								...
								<tr>
									...
									<td class="actions">
										<?= $this->Html->link(__(''),['action'=>'edit', $ps->id],['class'=>'fa fa-edit btn btn-warning btn-default btn-xs']) ?>
										<?= $this->Form->postLink(__(''),['action' => 'delete', $ps->id], ['confirm' => __('Está seguro de eliminar el registro # {0}?', $ps->id),'class'=>'fa fa-times btn btn-danger btn-default btn-xs']) ?>
									</td>
								</tr>
								<?php endforeach; ?>
								</tbody>
