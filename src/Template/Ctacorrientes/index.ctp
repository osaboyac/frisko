						...
						<div class="dataTable_wrapper">
							<table class="table table-striped table-bordered table-hover" id="dataTables-example">
								...
								<tr title="<?= $cc->descripcion ?>">
									...
									<td class="actions">
										<?= $this->Html->link(__(''),['action'=>'edit', $cc->id],['class'=>'fa fa-edit btn btn-warning btn-default btn-xs']) ?>
										<?= $this->Form->postLink(__(''),['action' => 'delete', $cc->id], ['confirm' => __('Está seguro de eliminar el registro # {0}?', $cc->id),'class'=>'fa fa-times btn btn-danger btn-default btn-xs']) ?>
									</td>
								</tr>
								<?php endforeach; ?>
								</tbody>
